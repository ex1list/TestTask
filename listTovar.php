  ...
  <table border="1">
    ...
    <?php foreach ( $arr['tovar']['results'] as $tovar ) { ?>
      <tr onclick="location='admin.php?action=editTovar&amp;tovarId=<?php echo $tovar->id?>'">
        <td>
          <?php if(isset ($tovar->id)) {
                  echo $tovar->id;                        
                } 
          ?>
        </td>
        <td>
          <?php if(isset ($tovar->Name)) {
                  echo  $tovar->Name; 
                } else {
                    echo "Без name";
                  }
